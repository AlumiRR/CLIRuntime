...
Тест проверяет работу парсинга и исполнения команд (command_utils) и сокета соединения (udp)
...
Сервер пишет логи в `/var/log/syslog`. Их также можно посмотреть через `systemctl status` если программа запущена как сервис.
